...
状态 DONE（独立模块交付）；角色/owner codex 独立模块实现者；基线 8272ac2d5f6afa938938d64b958559e76879e893；状态源本卡。用户明确要求修复生成、添加检查/手动更新/进度/安装后重启，沿用既有公开仓发布和本机部署授权。实现、测试、实际飞书生成、本机入口部署及 GitHub 公开发布已核验；现有旧进程由用户结束后重开，不强制打断专注。
...
发布核验：`v0.1.0-preview.20` 指向功能 commit `de902999e7501c25db93a69d8a255951a56e263b`，5 个资产均 uploaded、非 draft，远端散列与本地一致。生产默认 GitHub provider 无认证检查：preview.19 识别 preview.20 可更新，preview.20 显示最新。实际便携启动→下载→固定 NSIS 安装→自动重启另行通过，记录保留；正式 portable 内更新配置直接解包核对指向本仓 preview。
