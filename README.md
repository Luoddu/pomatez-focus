...
**当前版本：0.1.0-preview.3 · Windows x64 预览版。** 桌面计时与飞书记录同步已验证；飞书看板的图表需要按说明在飞书界面配置。尚未完成独立用户体验验收，不作为稳定版发布。
...
展开界面左侧计时，右侧同时显示今日／累计番茄、专注时长和按日期分组的记录。窗口使用 Windows 原生最小化、最大化／还原和关闭控件。**展开窗口不置顶，点击任务栏可正常切换应用；点击置顶图标会进入置顶小窗。** 小窗可取消置顶，展开后自动解除置顶。置顶操作不主动抢键盘焦点。
...
- Snipaste 截图后的置顶兼容修复已采用 Electron 官方窗口层级配置；隐藏测试不能证明真实截图覆盖层的完整交互，仍需用户实测。
