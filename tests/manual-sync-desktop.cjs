...
FocusService.prototype.archiveHistory = async (r) => {
  const archived = { ...r, sync: "synced", cloudSynced: true };
  shared = [archived];
  return archived;
};
...
FocusService.prototype.sync = async (r) => {
  ...
  if (calls === 1) throw Error("Synthetic offline");
  await new Promise((resolve) => (release = resolve));
  return { synced: true, planId: "p1", completedCount: 1 };
};
...
app.whenReady().then(async () => {
  try {
    ...
    const click = (t) =>
      js(
        `(()=>{const b=[...document.querySelectorAll('button')].find(b=>b.getAttribute('aria-label')===${JSON.stringify(
          ...
          t
        )});if(!b)throw Error('Missing button '+${JSON.stringify(
          t
        )});b.click()})()`
      );
    const stored = () =>
    ...
    await until(() =>
      js("Boolean(document.querySelector('[aria-label=\"手动同步\"]'))")
    );
    await until(() =>
      js(
    ...
    await until(() =>
      js(
        "document.querySelector('[role=alert]')?.textContent.includes('Synthetic offline')"
      )
    );
    ...
    assert.match(
      await js(
        "document.querySelector('[aria-label=\"手动同步\"]').textContent"
      ),
      /1/
    );
    await click("手动同步");
    ...
    assert.equal(
      await js(
        "document.querySelector('[aria-label=\"手动同步\"]').disabled"
      ),
      true
    );
    assert.equal(
    ...
    await until(() =>
      js(
        "document.querySelector('[aria-label=\"手动同步\"]').textContent==='手动同步'"
      )
    );
    ...
    assert.ok(
      await js(
        "[...document.querySelectorAll('button')].some(b=>b.textContent==='补记')"
      )
    );
    configured = false;
    ...
    assert.equal(
      await js(
        "document.querySelector('[aria-label=\"手动同步\"]').title"
      ),
      "请先连接飞书"
    );
    ...
    const result = {
      passed: 4,
      checks: [
        "manual entry fails visibly and preserves pending record",
        "adjacent manual retry syncs the existing record",
        "in-flight clicks cannot duplicate requests; empty queue still allows cloud download",
        "disconnected hint and existing manual-entry controls preserved",
      ],
    };
    fs.writeFileSync(
      path.join(__dirname, "../artifacts/manual-sync-test.json"),
