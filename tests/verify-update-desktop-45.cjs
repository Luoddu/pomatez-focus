// 端到端验证：真实 GitHub Provider + channel preview，检测刚发布的 preview.45。
// 复刻 FocusUpdater 的 NsisUpdater 配置（owner/repo/channel），
// 仅把“当前版本”覆盖为 0.1.0-preview.44，验证线上 Release 可被检测并取到元数据。
...
app.whenReady().then(async () => {
  try {
    ...
    // 当前版本伪装为 preview.44：若线上 45 可检测，必报 update-available
    updater.app = Object.create(updater.app);
    updater.app.getVersion = () => "0.1.0-preview.44";
    ...
    const ok =
      info &&
      info.version === "0.1.0-preview.45" &&
      (info.files || []).some((f) =>
        f.url.includes("Pomatez-Focus-v0.1.0-preview.45-win-x64-setup.exe")
      );
    console.log(ok ? "UPDATE DETECTED: 0.1.0-preview.45 OK" : "UPDATE CHECK FAILED");
